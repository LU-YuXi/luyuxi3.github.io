      ...
      <div class="py-container">
        ...
        <div class="bread">
          ...
          <ul class="fl sui-tag" >
            ...
            <!-- 关键字的面包屑 -->
            <li class="with-x" v-if="searchParams.keyword">{{searchParams.keyword}}<i @click="removeKword">x</i></li> 
            <!-- 品牌的面包屑 -->
...
import { mapGetters,mapState } from 'vuex';
  ...
  beforeMount() {
    ...
    computed:{
      // mapGetter里面的写法传递数组，因为getters计算属性没有划分模块（home，search)
    ...
    methods:{
      // 封装一个函数（需要时调用）向服务器发送请求获取search模块数据（根据参数不同放回不同的数据进行展示
     ...
     removeCategoryName(){
      ...
      //  如果把响应的字段写undefined，当前这个字段就不会带给服务器，提高性能
       this.searchParams.category3Id= undefined;
       this.getData();
         // 地址栏也需要修改，进行路由跳转      
        //  我们本意是删除query参数，但是如果有params参数就不删除所以要判断
    ...
    changeOrder(flag){
          // flag形参，是一个标记，戴表用户点击的综合（1）价格（2） 用户点击的时候传递过来
    ...
    watch:{
      ...
      $route(newValue,oldValue){
        ...
        //  再次发起Ajax请求
        this.getData();
        // 每一次请求完毕，因该 把响应的1.2.3级分类id制空，让他接受下一次的相应1.2.3  到53
        this.searchParams.category1Id=undefined;
        this.searchParams.category2Id=undefined;
        this.searchParams.category3Id=undefined;
      }
