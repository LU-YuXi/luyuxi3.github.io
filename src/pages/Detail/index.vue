        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            ...
            <div class="cartWrap">
              ...
              <div class="add">
              <!-- 加入购物车进行路由跳转之前，要发请求，把购买产品的信息通过请求新式，告诉服务器，服务器进行存储 -->
...
<script>
  ...
  export default {
    ...
    computed:{
      ...
      // 给子组件的数据
      skuImageList(){
        // 如果服务器的数据没有回来，skuInfo这个对象是一个空数组
          return this.skuInfo.skuImageList||[];
      }
    },
   methods:{
    changeActive(saleAttrValue,arr){
          // 遍历全部售卖属性值isChecked为0 没有高亮了 用排他思想
          arr.forEach(item => {
            item.isChecked =0;
          });
          // 点击的那个属性售卖值
    ...
    async addShopcar(){
      //  1：发请求，将产品加入数据库  
      //点击加入购物车这个按钮的时候，做的，第一件事件，将参数带个 服务器，（发请求），通知服务器加入购物车的产品是什么。
      
      // 在这里由于在这里，这个函数调用的 是仓库中的addorupdateshopcart函数，由于这个结果被返回到仓库中，是成功还是失败不知道，所以要判断
      // 当我们点击加入购物车 时需要判断数据是否成功给到数据库，需要知道成功还是失败。 
     
     try{
      //  成功
     await this.$store.dispatch('addOrUpdateShopCart',{skuId:this.$route.params.skuid,skuNum:this.skuNum}); 
    //  进行路由跳转
    // 在路由跳转的时候，还需要将产品的信息带给 下一级路由组件
    // 产品信息数据（比较复杂：skuinfo）通过会话存储（h5中新增的，另外一个叫本地存储）不持久化，会话结束数据就消失
    // 但是本地存储||会话存储，都不可以存储对象，一般存储字符串
    // 下面是使用会话存储
